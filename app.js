...
function sectorStatusLabel(status, lastReport) {
  if (status === 'pending') return `<i class="ti ti-circle-dashed" style="color:var(--text-3)"></i> Nunca enviou`;
  if (status === 'alert') {
...
function renderOverviewGrid(stats) {
  ...
  const statusColor = { ontime: '#1D9E75', late: '#EF9F27', critical: '#E24B4A', alert: '#F97316', pending: '#3c4455' };
  const statusLabel = { ontime: 'No prazo', late: 'Atrasado', critical: 'Crítico', alert: 'Em alerta', pending: 'Pendente' };
  const statusIcon  = { ontime: 'ti-circle-check', late: 'ti-clock-exclamation', critical: 'ti-alert-triangle', alert: 'ti-bell-exclamation', pending: 'ti-circle-dashed' };
...
const PANEL_CONFIG = [
  ...
  { key: 'pending',  label: 'Pendentes', icon: 'ti-circle-dashed',   color: '#5c6474', dimColor: 'rgba(255,255,255,0.05)', border: 'rgba(255,255,255,0.1)' },
];
