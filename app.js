...
function getStatus(submittedAt, refDate) {
  // Compare only dates, ignoring time, to avoid timezone issues
  const submittedDay = new Date(submittedAt).toLocaleDateString('en-CA'); // YYYY-MM-DD local
  const refDay = refDate.slice(0, 10); // YYYY-MM-DD
  const refPlus7 = new Date(refDate + 'T00:00:00');
  refPlus7.setDate(refPlus7.getDate() + 7);
  const refPlus7Day = refPlus7.toLocaleDateString('en-CA');

  if (submittedDay <= refDay) return 'ontime';
  if (submittedDay > refPlus7Day) return 'critical';
  return 'late';
}
