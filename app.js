...
const BUCKET = 'arquivos_de_relatório';
...
async function uploadFile(file) {
  ...
  if (!res.ok) {
    ...
    throw new Error('Erro no upload do arquivo: ' + err);
  }
...
function isOnTime(submittedAt, refDate) {
  return new Date(submittedAt) <= getWeekEnd(refDate);
}
...
function setWeekLabel() {
  const now = new Date();
  const s = getWeekStart(now), e = getWeekEnd(now);
...
async function submitReport() {
  ...
  try {
    ...
    const record = {
      ...
      file_url: fileUrl,
      on_time: isOnTime(now.toISOString(), data),
...
function renderDashboard() {
  ...
  const thisWeekSectors = new Set(
    reports.filter(r => getWeekStart(r.data).toISOString().split('T')[0] === weekKey).map(r => r.setor)
  );
...
function renderSectorGrid() {
  ...
  if (!reports.length) {
    grid.innerHTML = `<div class="empty-state" style="grid-column:1/-1">
      <i class="ti ti-upload"></i><p>Envie o primeiro report para ver o dashboard.</p></div>`;
    return;
  }
  const stats = SECTORS.map(sector => {
    const sr = reports.filter(r => r.setor === sector);
    const onTime = sr.filter(r => r.onTime).length;
    const late = sr.filter(r => !r.onTime).length;
    const total = sr.length;
    const pct = total > 0 ? Math.round((onTime / total) * 100) : null;
    return { sector, total, onTime, late, pct, lastReport: sr[0] };
  }).filter(s => s.total > 0).sort((a,b) => (b.pct ?? -1) - (a.pct ?? -1));

  if (!stats.length) {
    grid.innerHTML = `<div class="empty-state" style="grid-column:1/-1"><i class="ti ti-inbox"></i><p>Nenhum report ainda.</p></div>`;
    return;
  }
